...
// Finance Dashboard's / Food Stock's / HR-Linen's / Ops's CompanyContext.jsx
...
// own deploy. Maintenance only has the two-tier admin/staff role, same as
// Ops — no hr_admins-style extension needed.
//
...
// into Maintenance for it. Admins and platform admins always pass. A plain
// staff account with NO user_app_access rows at all for a company is
// legacy/unrestricted (predates this feature) and also passes; one with
// rows only passes if 'maintenance' is explicitly among them. Note this is
// a UI-level filter, not database-level enforcement — see
...
const STORAGE_KEY = 'maint_company_id'
const APP_KEY = 'maintenance'
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('companies').select('id, slug, name, status, member_billing_enabled, theme_accent, theme_mode').order('name'),
